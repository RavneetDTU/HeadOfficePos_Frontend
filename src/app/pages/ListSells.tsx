...
import type { Sell, SellListParams } from "../types/inventory";
import { printSellInvoice } from "../lib/printSellInvoice";
...
function InvoiceDetailModal({
    ...
    return (
      <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm">
        <div className="bg-white p-6 rounded-xl shadow-lg max-w-sm w-full">
          ...
          <p className="text-sm text-gray-600 mb-4">{error || "Sell details not found."}</p>
  ...
  const fromWh = warehouses.find((w) => w.id === sell.from_warehouse_id) || {};
  const toSt = stores.find((s) => s.id === sell.to_store_id) || {};

  const fromWhName = sell.from_warehouse_name || fromWh.name || "HEAD OFFICE WAREHOUSE";
  const toStName = sell.to_store_name || toSt.name || "Store Manager";

  const total = (sell.items || []).reduce(
    (acc: number, item: any) => acc + item.quantity * (item.purchase_price ?? item.purchasePrice ?? 0),
    0
  );

  const formattedDate = new Date(sell.sell_date).toLocaleString("en-ZA", {
    day: "2-digit",
    month: "2-digit",
    year: "numeric",
    hour: "2-digit",
  ...
  const handlePrint = () => {
    printSellInvoice({
      ...
      toPhone: toSt.phone || "—",
      toEmail: toSt.email || "—",
      biller: sell.created_by || "HeadOffice",
      lines: (sell.items || []).map((l: any) => ({
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 overflow-y-auto">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl overflow-hidden my-8">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-150 bg-gray-50">
          <h2 className="text-base font-bold text-gray-800 font-mono">Invoice: {sell.sell_reference}</h2>
        ...
        <div className="p-6 max-h-[75vh] overflow-y-auto space-y-6">
          <div className="border border-gray-200 rounded-lg p-5 bg-white shadow-sm space-y-6">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              <div className="flex gap-3 items-start">
                ...
                <div className="text-xs space-y-1">
                  <p className="text-gray-900 font-bold">Reference: {sell.sell_reference}</p>
                  <p className="text-gray-500">Date: {formattedDate}</p>
                  <p className="text-gray-500">Sell Status: <span className="font-semibold text-gray-700">{sell.status}</span></p>
            ...
            <div className="rounded-lg border border-gray-200 overflow-hidden">
              <table className="w-full text-xs">
                ...
                <tbody className="divide-y divide-gray-150">
                  {(sell.items || []).map((item: any, idx: number) => {
            ...
            <div className="bg-gray-50 border border-gray-150 rounded-lg p-3 text-xs space-y-1 w-fit">
              <p className="text-gray-700"><strong>Created by :</strong> {sell.created_by || "HeadOffice"}</p>
...
function StatusBadge({ status }: { status: Sell["status"] }) {
  const map: Record<Sell["status"], string> = {
...
export function ListSells() {
  ...
  const handleStatusChange = async (sellId: number, newStatus: string) => {
    const sell = sells.find((t) => t.id === sellId);
    if (!sell) return;

    const currentStatus = sell.status;
    if (currentStatus === newStatus) return;

    setUpdatingId(sellId);
    setError("");
    setSuccessToast("");
    ...
    try {
      ...
      if (newStatus === "Approved") {
        if (currentStatus === "Pending") {
          await updateSellStatus(sellId, "Approved");
        }
      } else if (newStatus === "In Transit") {
        if (currentStatus === "Pending") {
          await updateSellStatus(sellId, "Approved");
        }
        await updateSellStatus(sellId, "In Transit");
        ...
        await updateSellStatus(sellId, "Delivered");
      } else {
        await updateSellStatus(sellId, newStatus);
      }

      setSuccessToast(`Sell ${sell.sell_reference} updated to ${newStatus}!`);
      ...
      setError(e instanceof Error ? e.message : "Failed to update sell status on backend.");
  ...
  const load = useCallback(async () => {
    ...
    setError("");
    try {
      const params: SellListParams = { page, limit: LIMIT };
      if (statusFilter) params.status = statusFilter;
      const res = await getSells(params);

      let data = res.sells;
      if (debouncedSearch) {
        const s = debouncedSearch.toLowerCase();
        data = data.filter((t) =>
          t.sell_reference.toLowerCase().includes(s) ||
      ...
      setError(e instanceof Error ? e.message : "Failed to load sells");
    } finally {
      setLoading(false);
    }
  ...
  const completedCount = sells.filter((t) => t.status === "Completed").length;
            ...
            <tbody className="divide-y divide-gray-100">
              {loading ? (
                ...
                <tr><td colSpan={9} className="px-4 py-16 text-center">
                  ...
                  <p className="text-sm text-gray-400">No sells found</p>
                  <Link to="/sells/add" className="text-sm text-blue-600 hover:underline mt-1 inline-block">Create first sell →</Link>
                </td></tr>
              ) : sells.map((t, i) => (
                <tr key={t.id} className={`hover:bg-gray-50 transition-colors ${i % 2 === 0 ? "bg-white" : "bg-gray-50/50"}`}>
                  ...
                  <td className="px-4 py-3">
                    <span className="text-xs font-mono bg-blue-50 text-blue-700 px-2 py-0.5 rounded">{t.sell_reference}</span>
                  </td>
                  <td className="px-4 py-3 text-xs text-gray-600 whitespace-nowrap">{fmtDate(t.sell_date)}</td>
                  ...
                  <td className="px-4 py-3">
                    <button
                      onClick={() => setSelectedSellId(t.id)}
